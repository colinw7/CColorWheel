...
rgbToPos(const CRGBA &rgb, CIPoint2D &wpos, CIPoint2D &spos) const
{
  ...
  CHSV hsv = CRGBUtil::RGBtoHSV(rgb.getRGB());
...
setRGB(const CRGBA &rgb)
{
  if (mode_ == MODE_FOREGROUND) {
    fg_rgb_ = rgb;
    fg_hsv_ = CRGBUtil::RGBtoHSV(fg_rgb_.getRGB());
  }
  else {
    bg_rgb_ = rgb;
    bg_hsv_ = CRGBUtil::RGBtoHSV(bg_rgb_.getRGB());
  }
}
...
setHSV(const CHSV &hsv)
{
  if (mode_ == MODE_FOREGROUND) {
    fg_hsv_ = hsv;
    fg_rgb_ = CRGBUtil::HSVtoRGB(fg_hsv_);
  }
  else {
    bg_hsv_ = hsv;
    bg_rgb_ = CRGBUtil::HSVtoRGB(bg_hsv_);
  }
}
